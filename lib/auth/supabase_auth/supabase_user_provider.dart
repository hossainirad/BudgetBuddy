...
class BudgetBuddySupabaseUser extends BaseAuthUser {
  BudgetBuddySupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> budgetBuddySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BudgetBuddySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
